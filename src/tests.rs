...
mod serde_support {
    ...
    use crate::style::{ChatColor, ClickEvent, Style};

    use serde_json::json;

    #[test]
    fn test_serializing() {
        let test_obj = Chat::text(
            "Test String",
            Style::v1_16()
    ...
    fn test_deserializing() {
        let obj: Chat = serde_json::from_str(r#"{"translate":"chat.type.text","with":[{"text":"Herobrine","clickEvent":{"action":"suggest_command","value":"/msg Herobrine "},"hoverEvent":{"action":"show_entity","value":"{id:f84c6a79-0a4e-45e0-879b-cd49ebd4c4e2,name:Herobrine}"},"insertion":"Herobrine"},{"text":"I don't exist"}]}"#).unwrap();
        assert_eq!(
        ...
        let obj2: Chat = serde_json::from_str(r#"["",{"text":"Welcome to "},{"text":"Minecraft","color":"dark_blue"},{"text":" Tools","color":"green"}]"#).unwrap();
        assert_eq!(
